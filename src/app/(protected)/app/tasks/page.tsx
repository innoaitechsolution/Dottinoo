      ...
      try {
        ...
        if (profileData.role === 'teacher') {
          ...
          if (tasksError) {
            ...
            const errorDetails = tasksError.details ? ` (${tasksError.details})` : ''
            const errorHint = tasksError.hint ? ` Hint: ${tasksError.hint}` : ''
            setError(`Failed to load tasks: ${errorMsg}${errorDetails}${errorHint}`)
            setTasks([]) // Clear tasks on error
          } else {
            setTasks(tasksData || [])
            // Status is now included in the task data from listTasksForStudent
